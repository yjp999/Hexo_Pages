...
显然，点$$x_n$$处的切线方程为$$y=f(x_n)+f'(x_n)(x-x_n)$$，所以容易得到: $$x_{n+1}=x_{n}-\frac{f(x_n)}{f'(x_{n})}$$，求出点$$x_{n+1}$$的值后，和上面一样的步骤，可以得到过点$$(x_{n+1},f(x_{n+1}))$$的切线方程，于是又可以
得到点$$x_{n+2}$$的值，这样不断迭代下去，直至无限逼近零点（即曲线$$f(x)$$与$$x$$轴的交点）。
